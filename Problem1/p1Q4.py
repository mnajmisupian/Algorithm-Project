...
base = f'https://maps.googleapis.com/maps/api/staticmap?key={apikey}&size=640x640&scale=2&'
...
while(j < 3):
    ...
    while (i != 5):
        ...
        print(f'Driving distance from {originName[j]} -> {name} ->{destinationName[j]} :')
    ...
    print(f'The shortest distance is using {courierName[index]} with distance of {shortestDistance/ 1000} km')
    print()
    ...
    shortest_poly = polylines[shortestDistance] # Retrieve back the two polylines that make up the shortest route
